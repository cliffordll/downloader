...
class MultiColumnTreeModel(dv.PyDataViewModel):
    ...
    def GetParent(self, item):
        ...
        objs = self.ItemToObject(item)
        print("GetParent", objs, objs[:-1])
        if len(objs) == 1:
            return dv.NullDataViewItem
        elif len(objs) == 2:
            idxi = objs[0]
            return self.ObjectToItem((idxi, ))
            # return self.ObjectToItem(objs[:-1])
        else:
            pass
    ...
    #             # attr.SetAlignment(wx.ALIGN_RIGHT)
    #             attr.SetColour(wx.RED)
    #             # attr.SetColour(wx.Colour(100, 100, 100))  # 灰色
    #             return True
    #     return False
